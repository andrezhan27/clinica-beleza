...
export function TreatmentHero({ treatment, category }: { treatment: Treatment; category: TreatmentCategory }) {
  ...
  return (
    <>
      <nav className="treatment-breadcrumb" aria-label="Breadcrumb"><TreatmentsCatalogueLink>{language === "pt" ? "Tratamentos" : "Treatments"}</TreatmentsCatalogueLink><span>/</span><Link href={`/tratamentos/${category.slug}`}>{category.name[language]}</Link><span>/</span><span>{treatment.name[language]}</span></nav>
