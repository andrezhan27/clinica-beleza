...
export function Navbar() {
  ...
  return (
    <header className={`navbar ${compact ? "navbar--compact" : ""}`}>
      <a href="#inicio" className="brand" aria-label="Clínica Beleza — início"><Image src="/images/logo.svg" width={154} height={66} alt="Clínica Beleza" priority /></a>
      <nav className="desktop-nav" aria-label="Navegação principal">
        {links.map(([href, label]) => <a key={href} href={href}>{label}</a>)}
      </nav>
      <div className="nav-actions">
        <a className="phone-link" href="tel:+351210000000" aria-label="Ligar para a Clínica Beleza"><Phone size={15} /><span>+351 210 000 000</span></a>
      ...
      <div className={`mobile-menu ${open ? "mobile-menu--open" : ""}`} aria-hidden={!open}>
        <div className="mobile-menu__top"><Image src="/images/logo.svg" width={150} height={65} alt="Clínica Beleza" /><button onClick={() => setOpen(false)} aria-label={t.nav.close}><X size={25} /></button></div>
        ...
        <div className="mobile-menu__foot"><a href="tel:+351210000000">+351 210 000 000</a><p>Saldanha · Lisboa</p></div>
      </div>
    </header>
  );
}
